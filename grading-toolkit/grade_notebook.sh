...
uv run jq '.cells |= map(select(.metadata.tags and (.metadata.tags | index("'$cell_tag'"))))' $assignment_notebook > $ungraded_notebook
...
uv run jupytext $ungraded_notebook --to py

cat $ungraded_py $test_python_script > tmp_run.py
uv run python tmp_run.py || { echo "Assertion error occurred"; exit 1; }
rm tmp_run.py
